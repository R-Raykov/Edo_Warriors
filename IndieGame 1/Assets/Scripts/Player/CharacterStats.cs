...
public enum PlayerClass { Defaut, Samurai, Geisha };
...
public class CharacterStats : MonoBehaviour
{
    ...
    [SerializeField] private float _mana = 200;
    ...
    private void Update()
    {
        if (Input.GetKeyDown(KeyCode.N)) GameManager.Instance.RegisterPlayer(this);

        if (Input.GetKeyDown(KeyCode.G))
        {
            if (CharacterClass == PlayerClass.Samurai)     CharacterClass = PlayerClass.Geisha;
            else if (CharacterClass == PlayerClass.Geisha) CharacterClass = PlayerClass.Samurai;
        }
    ...
    {
        Transform spawn = GameObject.FindObjectOfType<Checkpoint>().transform;

        if (PlayerNumber == 1) transform.position = spawn.GetChild(0).transform.position;
        else if (PlayerNumber == 2) transform.position = spawn.GetChild(1).transform.position;
    }
}